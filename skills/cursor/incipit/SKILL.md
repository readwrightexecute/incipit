---
name: incipit
description: Turn a rough software idea into a single structured Implementation Brief (mega-prompt) via a fast elicitation flow. Use when the user has a half-formed software idea and wants a rigorous, paste-ready spec before building.
---
...
**Don't use it for:** already-specced work that just needs coding; non-software
...
a correction (don't interrogate — this is inference, not a questionnaire):
...
### 3. Clarify
Ask the **N most decision-changing questions** (N from the table above) — the ones
whose answers would most change how this gets built. For each, provide a sensible
default prefixed with `[ASSUMPTION]`. Present them exactly like this:
...
`[ASSUMPTION]`**. Record each answer (or the assumption) — you'll thread them into
every section.
...
See **Section definitions** below for each section's exact instruction.

After each section, offer the **Refine** menu (below) — but don't block on it; most
sections are fine as drafted.
